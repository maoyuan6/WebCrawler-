...
using DCSoft.Design;
using Clinic.Case.Business.Properties;
using HIS.Clinic.ClinicCase;
using WindowsFormsApp1.Properties;
//using iNethinkPE.Helper;

namespace Clinic.Case.Business
{
    public partial class CaseFrm : UserControl
    {
        ...
        {//第1-5医保码
            ...
            dtdia.Columns.Add("Code");
            dtdia.Columns.Add("Name");
            string[] strs = value_name == "" ? new string[] { } : value_name.Split(new char[] { '^' });
            foreach (string stritem in strs)
            {
                if (stritem != "")
                {
                    string[] strs2 = stritem.Split(new char[] { '#' });
                    DataRow dr = dtdia.NewRow();
                    dr["Code"] = strs2[0];
                    dr["Name"] = strs2[1];
                    dtdia.Rows.Add(dr);
                }
            }
            XTextElementList XTextList = this.myWriterControl.GetElementsByName("XYZhenDuan");
            foreach (XTextElement item in XTextList)
            {
                XTextInputFieldElement inputcode = item as XTextInputFieldElement;
                if (inputcode != null)
                {
                    inputcode.TagValue = dtdia;
                }
            }

        }
        public string ReturnMedicalCode()
        {
            string value_name = "";
            XTextElementList XTextList = this.myWriterControl.GetElementsByName("XYZhenDuan");
            foreach (XTextElement item in XTextList)
            {
                ...
                if (inputcode != null)
                {
                    DataTable dt = inputcode.TagValue as DataTable;
                    if (dt != null)
                    {
                        foreach (DataRow dr in dt.Rows)
                        {
                            value_name += dr["Code"] + "#" + dr["Name"] + "^";
                        }
                    }
                    break;
                }
            }
            return value_name == "" ? "" : value_name.Substring(0, value_name.Length - 1);
        }
        #endregion
        #region 中医诊断
        public void ChineseMedicineIcdEditValue(string value_name)
        {//第1-5医保码
            DataTable dtdia = new DataTable();
            dtdia.Columns.Add("Code");
            dtdia.Columns.Add("Name");
            ...
            XTextElementList XTextList = this.myWriterControl.GetElementsByName("ZYZhenDuan");
            foreach (XTextElement item in XTextList)
            {
                XTextInputFieldElement inputcode = item as XTextInputFieldElement;
                if (inputcode != null)
        ...
        public string ReturnChineseMedicine()
        {
            string value_name = "";
            XTextElementList XTextList = this.myWriterControl.GetElementsByName("ZYZhenDuan");
            foreach (XTextElement item in XTextList)
        ...
        public void SaveModeHeader()
        {
            ...
            XmlNamespaceManager xmlm = new XmlNamespaceManager(xmlDoc.NameTable);//将管理器指定到文档
            xmlm.AddNamespace("xsi", "http://www.w3.org/2001/XMLSchema-instance");//添加命名空间

            XmlNode targetNode = root.SelectSingleNode("XElements/Element[@xsi:type='XTextHeader']", xmlm);//获得目标节点
            if (targetNode == null)
            {
                MessageBox.Show("没有找到");
                return;
            }
            ModeInsert modeFrm = new ModeInsert("页眉添加");
            empTempletHeader = WinnerHIS.Clinic.Case.DAL.Interface.DALHelper.DALManager.CreateEMRTEMPLET_HEADER();
            empTempletHeader.Session = CacheHelper.Session;
            if (true)
            {
                if (modeFrm.ShowDialog() == DialogResult.OK)
                {
                    empTempletHeader.CREATOR_ID = ContextHelper.Employee.EmployeeID;
                    empTempletHeader.CREATE_DATETIME = ContextHelper.CurrentTime;
                    empTempletHeader.NAME = modeFrm.Name;
                }
            }
            empTempletHeader.LAST_TIME = ContextHelper.CurrentTime;
            empTempletHeader.CONTENT = targetNode.InnerXml;
            empTempletHeader.Save();
            MessageBox.Show("页眉新增成功！");
        }
        public void SaveModeFooter()
        {
            string ss = myWriterControl.XMLText;
            XmlDocument xmlDoc = new XmlDocument();
            xmlDoc.LoadXml(ss);
            ...
            XmlNode targetNode = root.SelectSingleNode("XElements/Element[@xsi:type='XTextFooter']", xmlm);//获得目标节点
            if (targetNode == null)
            {
                MessageBox.Show("没有找到");
                return;
            }
            ModeInsert modeFrm = new ModeInsert("页脚添加");

            empTempletfoot = WinnerHIS.Clinic.Case.DAL.Interface.DALHelper.DALManager.CreateEMRTEMPLET_FOOT();
            empTempletfoot.Session = CacheHelper.Session;
            if (true)
            {
                if (modeFrm.ShowDialog() == DialogResult.OK)
                {
                    empTempletfoot.CREATOR_ID = ContextHelper.Employee.EmployeeID;
                    empTempletfoot.CREATE_DATETIME = ContextHelper.CurrentTime;
                    empTempletfoot.NAME = modeFrm.Name;
                }
            }
            empTempletfoot.LAST_TIME = ContextHelper.CurrentTime;
            empTempletfoot.CONTENT = targetNode.InnerXml;
            empTempletfoot.Save();
            MessageBox.Show("页脚新增成功！");
        }
        public void SaveSunMode()
        {
        ...
        private void btnInsertButton_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            myWriterControl.ExecuteCommand("InsertButton", true, null);
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void barBtnSign_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            Image image = Resources.sign;
            myWriterControl.ExecuteCommand("InsertImageExt", false, image);
